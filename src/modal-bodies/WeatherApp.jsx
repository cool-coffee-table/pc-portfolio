...
export default function WeatherApp() {
  ...
  return (
    <div className="p-4  mx-auto w-[400px]">
      <div className="flex flex-col items-center justify-center w-full">
        <p className="text-2xl underline mb-2">Current Weather</p>
        <input
          type="text"
          className="bg-[#3a3a3a] mt-1 text-lg max-w-[210px] outline-none py-0.5 px-2 rounded-md text-center"
      ...
      {weather && (
        <div className="mt-4">
          <p className="text-center text-6xl">{kelvinToF(weather.main.temp)}°F</p>
          <p className="text-center text-xl mt-1">{weather.weather[0].description}</p>
          <div className="flex mt-6 flex-wrap justify-between">
            <p className="mb-2">Humidity: {weather.main.humidity}%</p>
            <p className="mb-2">Feels Like: {kelvinToF(weather.main.feels_like)}°F</p>
          ...
          <p className="text-center mt-2">Last Updated: {convertUnixTo12HourFormat(weather.dt)}</p>
        </div>
      )}
    </div>
  );
